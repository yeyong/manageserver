    ...
    public class HttpModule : System.Web.IHttpModule
    {
        ...
        public void Application_OnError(Object sender, EventArgs e)
        {
            ...
            //if (context.Server.GetLastError().GetBaseException() is MyException)
            {
                ...
                context.Response.Write("SAS!NT Error:<br />");
        ...
        private void ReUrl_BeginRequest(object sender, EventArgs e)
        {
            ...
            //    string[] path = requestPath.Replace(BaseConfigs.GetSitePath, "/").Split('/');

            //    //当使用伪aspx, 如:/版块别名/1(分页)等.
            //    if (path.Length > 1 && !Utils.StrIsNullOrEmpty(path[1]))
            //    {
            //        int forumid = 0;
            //        foreach (SAS.Entity.ForumInfo foruminfo in Forums.GetForumList())
            //        {
            ...
            //        }
            //    }
            //    context.Response.Redirect(baseconfig.Forumpath + "tools/error.htm?templatepath=" + Templates.GetTemplateItem(Utils.StrToInt(strTemplateid, 0)).Directory + "&msg=" + Utils.UrlEncode("您请求的版块信息无效!"));
            //    return;
            //}
                    ...
                    if (config.BrowseCreateTemplate == 1)
                    {
                        if (requestPath.IndexOf("showtemplate.aspx") != -1)
                        {
                            CreateTemplate(forumPath,
                                Templates.GetTemplateItem(SASRequest.GetInt("templateid", 1)).Directory,
                                config.Indexpage == 0 ? "forumindex.aspx" : "website.aspx",
                                SASRequest.GetInt("templateid", 1)); //当跳转模板页时，生成目标文件
                        }
        ...
        public string SelectTemplate(string strTemplateid, string pagename, string newUrl)
        {
            ...
            //            {
            //                SAS.Entity.TopicInfo topicinfo = Topics.GetTopicInfo(Utils.StrToInt(urlvalue.Split('=')[1], 0));
            //                //主题存在时
            //                if (topicinfo != null)
            //                {
            ...
            //            else
            //            {
            //                forumid = SASRequest.GetInt("forumid", 0);
            //            }
            //        }
        ...
        public class SiteUrls
        {
            ...
            string SiteUrlsFile = HttpContext.Current.Server.MapPath(BaseConfigs.GetSitePath + "config/urls.config");
