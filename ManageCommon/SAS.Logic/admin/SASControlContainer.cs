...
namespace SAS.Logic
{
    ...
    /// 后台控制容器类
    /// </summary>
    public class SASControlContainer
    {
        private static SASControlContainer instance = null;

        private Container _container = null;

        private static object syncRoot = new object();

        private SASControlContainer()
        {
        ...
        public static SASControlContainer GetContainer()
        {
            if (instance == null)
            {
                lock (syncRoot)
                {
                    if (instance == null)
                    {
                        instance = new SASControlContainer();
                    }
                }
            }
            return instance;
        ...
        {
            instance = GetContainer();
            foreach (ISASNTComponent component in instance.CurrentContainer.Components)
            {
    ...
    //DISCUZNT 组件接口
    public interface ISASNTComponent : IComponent
    {
    ...
    public class NormalComponent : Component, ISASNTComponent
    {
        ...
        public NormalComponent(string componentName, object componentDataObject)
            : this(componentName)
        {
