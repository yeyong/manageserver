    ...
    public partial class forum_editforums : AdminPage
    {
        ...
        public void BindPower_Click(object sender, EventArgs e)
        {
            ...
            if (UserList.Text != "")
            {
                ...
                {
                    string uid = SAS.Logic.Users.GetUserId(adduser).ToString();
                    if (uid == "-1")
                        continue;
                    bool find = false;
        ...
        private void DataGridBind(string userList)
        {
            ...
            foreach (string user in userList.Split(','))
            {
                if (user.Trim() == "")
                    continue;
                int uid = SAS.Logic.Users.GetUserId(user);
                if (uid != -1)
                {
            ...
            if (Permuserlist != null)
            {
                foreach (string p in Permuserlist.Split('|'))
                {
                    ...
                    dr["uid"] = p.Split(',')[1];
                    dr["name"] = p.Split(',')[0];
                    dr["viewbyuser"] = power & (int)SAS.Entity.ForumSpecialUserPower.ViewByUser;
                    dr["postbyuser"] = power & (int)SAS.Entity.ForumSpecialUserPower.PostByUser;
                    dr["replybyuser"] = power & (int)SAS.Entity.ForumSpecialUserPower.ReplyByUser;
                    dr["getattachbyuser"] = power & (int)SAS.Entity.ForumSpecialUserPower.DownloadAttachByUser;
                    dr["postattachbyuser"] = power & (int)SAS.Entity.ForumSpecialUserPower.PostAttachByUser;
                    dt.Rows.Add(dr);
                }
            }
        ...
        private string GetTopicType()
        {
            ...
            {
                #region
                if (SASRequest.GetFormString("type" + i) == "") //循环处理选择的主题分类
                    break;
                else
                {
                    if (SASRequest.GetFormString("type" + i) != "-1")   //-1不使用，0平板显示，1下拉显示
                    {
                        string oldtopictype = SASRequest.GetFormString("oldtopictype" + i); //旧主题分类
                        string newtopictype = SASRequest.GetFormString("type" + i); //新主题分类
                    ...
                    {
                        if (SASRequest.GetFormString("oldtopictype" + i) != "")
                            tmpType = tmpType.Replace(SASRequest.GetFormString("oldtopictype" + i), "");
                    }
                }
                #endregion
                i++;
        ...
        private string GetPermuserlist()
        {
            ...
            foreach (object o in SpecialUserList.GetKeyIDArray())
            {
                ...
                if (SpecialUserList.GetCheckBoxValue(row, "viewbyuser"))
                    power |= (int)SAS.Entity.ForumSpecialUserPower.ViewByUser;
                if (SpecialUserList.GetCheckBoxValue(row, "postbyuser"))
                    power |= (int)SAS.Entity.ForumSpecialUserPower.PostByUser;
                if (SpecialUserList.GetCheckBoxValue(row, "replybyuser"))
                    power |= (int)SAS.Entity.ForumSpecialUserPower.ReplyByUser;
                if (SpecialUserList.GetCheckBoxValue(row, "getattachbyuser"))
                    power |= (int)SAS.Entity.ForumSpecialUserPower.DownloadAttachByUser;
                if (SpecialUserList.GetCheckBoxValue(row, "postattachbyuser"))
                    power |= (int)SAS.Entity.ForumSpecialUserPower.PostAttachByUser;
                ...
                if (!find)
                {
                    ...
                    result = ((shortUserInfo != null) ? shortUserInfo.Ps_name.Trim() : "") + "," + uid + "," + power + "|" + result;
                }
                row++;
            }
            if (result == "")
        ...
        private void InitializeComponent()
        {
            ...
            this.TabControl1.SelectedIndex = SASRequest.GetInt("tabindex", 0);
            ...
            LoadCurrentForumInfo(SASRequest.GetInt("fid", -1));
