...
namespace SAS.ManageWeb.ManagePage
{
    public partial class taobao_addbrandrecommend : AdminPage
    {
        ...
        protected void AddRecommendInfo_Click(object sender, EventArgs e)
        {
            ...
            if (errmsg != "")
            {
                base.RegisterStartupScript("", "<script>alert('" + errmsg + "');window.location.href='taobao_addbrandrecommend.aspx';</script>");
                return;
            }

            if (tpb.CreateRecommendInfo(thercategory, therchanel, thertitle, thecontent, rtype) > 0)
            {
                SAS.Cache.SASCache.GetCacheService().RemoveObject("/SAS/BrandList/Chanel_" + therchanel + "/Class_" + thercategory);
