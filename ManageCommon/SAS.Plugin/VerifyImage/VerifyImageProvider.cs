...
namespace SAS.Plugin.VerifyImage
{
    /// <summary>
    /// 验证码图片创建类
    /// </summary>
            ...
            if (!_instance.ContainsKey(assemlyName))
            {
                lock (lockHelper)
                {
                    if (!_instance.ContainsKey(assemlyName))
                    {
                        ...
                        try
                        {
                            p = (IVerifyImage)Activator.CreateInstance(Type.GetType(string.Format("SAS.Plugin.VerifyImage.{0}.VerifyImage, SAS.Plugin.VerifyImage.{0}", assemlyName), false, true));
                        }
                        catch
                        {
                            p = new SAS.Plugin.VerifyImage.JpegImage.VerifyImage();
                        }
