...
using SAS.Web.Services.API;
using SAS.Config;
using SAS.Web.Services.API.Actions;
using SAS.Common;
using SAS.Logic;
...
namespace SAS.Web.Services
{
    public class RESTServer : Page
    {
        ...
        void RESTServer_Load(object sender, EventArgs e)
        {
            ...
            //check sig
            SASParam[] parameters = GetParamsFromRequest(Request);
            ...
            //format
            string format = SASRequest.GetString("format");
            //callback
            string callback = SASRequest.GetString("callback");


            /*---- required ----*/

            //api_key
            string api_key = SASRequest.GetString("api_key");
            //整合程序对象
            ApplicationInfo appInfo = null;
            ApplicationInfoCollection appcollection = apiInfo.AppCollection;
            foreach (ApplicationInfo newapp in appcollection)
            {
                if (newapp.APIKey == SASRequest.GetString("api_key"))
                {
                    appInfo = newapp;
                }
            }
            ...
            string session_key = SASRequest.GetString("session_key");
            int uid = GetUidFromSessionKey(session_key, appInfo.Secret);




            string method = SASRequest.GetString("method");
            ...
            try
            {
                Type type = Type.GetType(string.Format("SAS.Web.Services.API.Actions.{0}, SAS.Web.Services", classname), false, true);
                ...
                double.TryParse(SASRequest.GetString("call_id"), out callid);
                if (callid > -1)
                {
        ...
        {
            string format = SASRequest.GetString("format").Trim().ToLower();
        ...
        private SASParam[] GetParamsFromRequest(HttpRequest request)
        {
            List<SASParam> list = new List<SASParam>();
            foreach (string key in request.QueryString.AllKeys)
            {
                list.Add(SASParam.Create(key, request.QueryString[key]));
            }
            foreach (string key in request.Form.AllKeys)
            {
                list.Add(SASParam.Create(key, request.Form[key]));
            }
            list.Sort();
            return list.ToArray();
        }
        ...
        private string GetSignature(SASParam[] parameters, string secret)
        {
            StringBuilder values = new StringBuilder();

            foreach (SASParam param in parameters)
            {
