...
namespace SAS.Control
{
    ...
    public class DataGrid : System.Web.UI.WebControls.DataGrid, INamingContainer
    {
        ...
        /// 跳转按钮
        /// </summary>
        public SAS.Control.Button GoToPagerButton = new SAS.Control.Button();
                ...
                if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
                {
                    ...
                    for (int i = 0; i < e.Item.Cells.Count; i++)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                //加载discuz下拉控件
                                if (c is SAS.Control.DropDownList)
                                {
                                    SAS.Control.DropDownList dropDownList = (SAS.Control.DropDownList)c;
        ...
        public System.Collections.ArrayList GetKeyIDArray()
        {
            ...
            if (SASRequest.GetString("keyid") != "")
            {
                foreach (string id in SASRequest.GetString("keyid").Split(','))
                {
                    al.Add(id);
                }
            }
        ...
        {
            return SASRequest.GetFormString(this.ClientID.Replace("_", ":") + ":_ctl" + (controlnumber + 3) + ":" + fieldname);
        }
