    ...
    public class SoftInfo
    {
        ...
        static SoftInfo()
        {
            if (serversoftware == null)
            {
                ...
                forumname = GeneralConfigs.GetConfig().Sitetitle;
            }
        }
        ...
        public static void SendPostData()
        {
            try
            {
                ...
                //得到系统相关信息
                int member = Convert.ToInt32(Statistics.GetStatisticsRowItem("st_usercount"));
                int topcis = Convert.ToInt32(Statistics.GetStatisticsRowItem("st_productcount"));
                int posts = Convert.ToInt32(Statistics.GetStatisticsRowItem("st_adcount"));

                string build = string.Empty;
                string strPath = Utils.GetMapPath(BaseConfigs.GetSitePath.ToLower() + "config/localupgradeini.config");
                ...
                int dbtype = 0;
                switch (SAS.Config.BaseConfigs.GetDbType.ToLower())
                {
                    case "sqlserver":
                        {
                            dbtype = 0;
                ...
                int forummajor = FileVersionInfo.GetVersionInfo(Assembly.GetExecutingAssembly().Location).ProductMajorPart;
                int forumminor = FileVersionInfo.GetVersionInfo(Assembly.GetExecutingAssembly().Location).ProductMinorPart;
                int forumrevision = FileVersionInfo.GetVersionInfo(Assembly.GetExecutingAssembly().Location).ProductBuildPart;
                csi.SetupSoftInfo(forumname, member, topcis, posts, serversoftware, forummajor, forumminor, forumrevision, dotnetmajor, dotnetminor, dotnetbuild, dbtype, build, osversion, serverip, servername);
