    ...
    public class UBB
    {
        ...
        public static string UBBToHTML(PostpramsInfo _postpramsinfo)
        {
            ...
            if (_postpramsinfo.Bbcodeoff == 0)
            {
                ...
                if (_postpramsinfo.Bbcodemode == 1)
                {
                    ...
                    for (m = r[1].Match(sDetail); m.Success; m = m.NextMatch())
                    {
                        ...
                        {
                            attachPath = BaseConfigs.GetSitePath + "upload/" + attachPath;
                        }
                        else
                        {
                            attachPath = BaseConfigs.GetSitePath + attachPath;
                        }
                    ...
                    for (m = r[5].Match(sDetail); m.Success; m = m.NextMatch())
                    {
                        ...
                        {
                            replacement = BaseConfigs.GetSitePath + "upload/" + replacement;
                        }
                        else
                        {
                            replacement = BaseConfigs.GetSitePath + replacement;
                        }
                        ...
                        replacement = string.Format(attachCode, attachIcon, BaseConfigs.GetSitePath + m.Groups[2].ToString().ToLower().Replace("viewfile.asp?id", "attachment.aspx?attachmentid"), replacement);
                        sDetail = sDetail.Replace(m.Groups[0].ToString(), replacement);
                    }
                    ...
                    for (m = r[7].Match(sDetail); m.Success; m = m.NextMatch())
                    {
                        ...
                        replacement = BaseConfigs.GetSitePath + m.Groups[2].ToString().ToLower().Replace("viewfile.asp?id", "attachment.aspx?attachmentid");
                        if (replacement.IndexOf("attachment.aspx?attachmentid") == -1)
                        {
                            replacement = BaseConfigs.GetSitePath + "upload/" + replacement;
                        }
            ...
            if (_postpramsinfo.Showimages == 1)
            {
                ...
                {
                    sDetail = Regex.Replace(sDetail, @"\[img\]\s*(http://[^\[\<\r\n]+?)\s*\[\/img\]", "<img src=\"$1\" border=\"0\" onload=\"thumbImg(this)\" />", options);
