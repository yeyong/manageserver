...
{
    public partial class ajaxbrandlist : System.Web.UI.UserControl
    {
        protected DataTable brandlist = new DataTable();
        ...
        public string brandname = SASRequest.GetString("brandname").Trim();
        public int relateclass = SASRequest.GetInt("relateclass", 0);

        public ajaxbrandlist()
        {
            ...
            string conditions = tpb.GetGoodsBrandSearchCondition(true, brandname, relateclass.ToString(), 1);
            int recordcount = tpb.GetGoodsBrandCountByCond(conditions);
            brandlist = tpb.GetGoodsBrandByPage(conditions, pagesize, currentpage);
        ...
        public string AjaxPagination(int recordcount, int pagesize, int currentpage)
        {
            if (SASRequest.GetInt("postnumber", 0) > 0)
            {
                return AjaxPagination(recordcount, pagesize, currentpage, "../usercontrols/ajaxbrandlist.ascx", "brandname=" + brandname + "&relateclass=" + relateclass + "&postnumber=" + SASRequest.GetInt("postnumber", 0), "taobaobrandlistgrid");
            }
            else
            {
                return AjaxPagination(recordcount, pagesize, currentpage, "../usercontrols/ajaxbrandlist.ascx", "brandname=" + brandname + "&relateclass=" + relateclass, "taobaobrandlistgrid");
            }
        }
