...
namespace SAS.ManageWeb.ManagePage
{
    public partial class forum_forumstree : SAS.Web.UI.AdminPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                if (!AdminForums.MovingForumsPos(SASRequest.GetString("currentfid"), SASRequest.GetString("targetfid"), SASRequest.GetString("isaschildnode") == "1" ? true : false))
                {
                    base.RegisterStartupScript("", "<script>alert('当前版块下面有子版块,因此无法移动!');window.location.href='forum_forumsTree.aspx';</script>");
                }
                AdminVistLogs.InsertLog(this.userid, this.username, this.usergroupid, this.grouptitle, this.ip, "移动论坛版块", "移动论坛版块ID:" + SASRequest.GetString("currentfid") + "到ID:" + SASRequest.GetString("targetfid"));
            }
            ...
            if (layer == 0)
            {
                ...
                for (int n = 0; n < drs.Length; n++)
                {
                    ...
                    str += "{fid:" + drs[n]["fid"].ToString() + ",name:\"" +
                           ...
                           drs[n]["subforumcount"].ToString() + ",istrade:" + 0 + "},\r\n";
            ...
            else
            {
                ...
                for (int n = 0; n < drs.Length; n++)
                {
                    ...
                    str += "{fid:" + drs[n]["fid"].ToString() + ",name:\"" +
                          ...
                          drs[n]["layer"].ToString() + ",subforumcount:" + drs[n]["subforumcount"].ToString() + ",istrade:" + 0 +
                          "},\r\n";
