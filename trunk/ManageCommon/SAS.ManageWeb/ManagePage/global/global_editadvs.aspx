<%@ Page Language="C#" CodeBehind="global_editadvs.aspx.cs" Inherits="SAS.ManageWeb.ManagePage.global_editadvs" %>
<%@ Register TagPrefix="yy1" TagName="TextareaResize" Src="../UserControls/TextareaResize.ascx" %>
...
<form id="Form1" method="post" runat="server">
	<div id="adtype1" style="width: 100%; display: none">
		<yy1:PageInfo ID="info1" runat="server" Icon="information" Text="<ul><li>展现方式: 头部横幅广告显示于论坛页面右上方, 通常使用 468x60 图片或 Flash 的形式. 当前页面有多个头部横幅广告时, 系统会随机选取其中之一显示.  </ul>  <ul><li>价值分析: 由于能够在页面打开的第一时间将广告内容展现于最醒目的位置, 因此成为了网页中价位最高、最适合进行商业宣传或品牌推广的广告类型之一. </ul>" />
	</div>
	<div id="adtype2" style="width: 100%; display: none">
		<yy1:PageInfo ID="PageInfo1" runat="server" Icon="information" Text="<ul><li>展现方式: 尾部横幅广告显示于论坛页面中下方, 通常使用 468x60 或其他尺寸图片、Flash 的形式. 当前页面有多个尾部横幅广告时, 系统会随机选取其中之一显示. </ul>  <ul><li>价值分析: 与页面头部和中部相比, 页面尾部的展现机率相对较低, 通常不会引起访问者的反感, 同时又基本能够覆盖所有对广告内容感兴趣的受众, 因此适合中性而温和的推广. </ul>" />
	</div>
	<div id="adtype3" style="width: 100%; display: none">
		<yy1:PageInfo ID="PageInfo2" runat="server" Icon="information" Text="<ul><li>展现方式: 页内文字广告以表格的形式, 显示于首页、主题列表和帖子内容三个页面的中上方, 通常使用文字的形式, 也可使用小图片和 Flash. 当前页面有多个文字广告时, 系统会以表格的形式按照设定的显示顺序全部展现, 同时能够对表格列数在 3~5 的范围内动态排布, 以自动实现最佳的广告排列效果.  </ul>  <ul><li>价值分析: 由于此类广告通常以文字形式展现, 但其所在的较靠上的页面位置, 使得此类广告成为了访问者必读的内容之一. 同一页面可以呈现多达十几条文字广告的特性, 也决定了它是一种平民化但性价比较高的推广方式, 同时还可用于论坛自身的宣传和公告之用. </ul>" />
	</div>
	<div id="adtype4" style="width: 100%; display: none">
		<yy1:PageInfo ID="PageInfo3" runat="server" Icon="information" Text="<ul><li>展现方式: 帖内广告显示于帖子标题的上方, 通常使用文字的形式. 当前页面有多个帖内广告时, 系统会从中抽取与每页帖数相等的条目进行随机显示. 您可以在 Discuz!NT 选项中的其他设置中修改每帖显示的广告数量.  </ul>  <ul><li>价值分析: 由于帖子是论坛最核心的组成部分, 位于帖子内容上方的帖内广告, 便可在用户浏览帖子内容时自然的被接受, 加上随机播放的特性, 适合于特定内容的有效推广, 也可用于论坛自身的宣传和公告之用. 建议设置多条帖内广告以实现广告内容的差异化, 从而吸引更多访问者的注意力. </ul>" />
	</div>
	<div id="adtype5" style="width: 100%; display: none">
		<yy1:PageInfo ID="PageInfo4" runat="server" Icon="information" Text="<ul><li>展现方式: 漂浮广告展现于页面左下角, 当页面滚动时广告会自行移动以保持原来的位置, 通常使用小图片或 Flash 的形式. 当前页面有多个漂浮广告时, 系统会随机选取其中之一显示.  </ul>  <ul><li>价值分析: 漂浮广告是进行强力商业推广的有效手段, 其在页面中的浮动性, 使其与固定的图片和文字相比, 更容易被关注, 正因为如此, 这种强制性的关注也可能招致对此广告内容不感兴趣的访问者的反感. 请注意不要将过大的图片或 Flash 以漂浮广告的形式显示, 以免影响页面阅读. </ul>" />
	</div>
	<div id="adtype6" style="width: 100%; display: none">
		<yy1:PageInfo ID="PageInfo5" runat="server" Icon="information" Text="<ul><li>展现方式: 对联广告以长方形图片的形式显示于页面顶部两侧, 形似一幅对联, 通常使用宽小高大的长方形图片或 Flash 的形式. 对联广告一般只在使用像素约定主表格宽度的情况下使用, 如使用超过 90% 以上的百分比约定主表格宽度时, 可能会影响访问者的正常流量. 当访问者浏览器宽度小于 800 像素时, 自动不显示此类广告. 当前页面有多个对联广告时, 系统会随机选取其中之一显示.  </ul>  <ul><li>价值分析: 对联广告由于只展现于高分辨率(1024x768 或更高)屏幕的两侧, 只占用页面的空白区域, 因此不会招致访问者反感, 能够良好的突出推广内容. 但由于对分辨率和主表格宽度的特殊要求, 使得广告的受众比例无法达到 100%. </ul>" />
	</div>
	<div id="adtype7" style="width: 100%; display: none">
		<yy1:PageInfo ID="PageInfo6" runat="server" Icon="information" Text="<ul><li>展现方式: Silverlight媒体广告以视频、图片、文字相结合的方式进行展现,通常从屏幕右下角浮现出来.  </ul>  <ul><li>价值分析: 视频背后是大图片广告,视觉冲击效果配合文字描述会带来意想不到的展现效果,是最有效的广告形式. </ul>" />
	</div>
	<div id="adtype8" style="width: 100%; display: none">
		<yy1:PageInfo ID="PageInfo7" runat="server" Icon="information" Text="<ul><li>展现方式: 帖间通栏广告显示于楼主和2楼帖子之间, 通常使用 728*90 图片或 Flash 的形式. 当前页面有多个帖间通栏广告时, 系统会随机选取其中之一显示.  </ul>  <ul><li>价值分析: 对帖子的质量有举足轻重作用的楼主与沙发之间的黄金地段, 往往容易受到访问者的大量关注. </ul>" />
	</div>
	<div id="adtype9" style="width: 100%; display: none">
		<yy1:PageInfo ID="PageInfo8" runat="server" Icon="information" Text="<ul><li>展现方式: 分类间广告显示于论坛分类之间  </ul>  <ul><li>价值分析: 当论坛分类过多时, 可能对访问者造成困扰. </ul>" />
	</div>
	<div id="adtype10" style="width: 100%; display: none">
		<yy1:PageInfo ID="PageInfo9" runat="server" Icon="information" Text="<ul><li>展现方式: 快速发帖栏上方广告显示于发帖框附近  </ul>  <ul><li>价值分析: 当用户想要发帖时必然会看到此类型的广告, 是十分有效的展示方式 </ul>" />
	</div>
...
<table width="100%">
	...
	<tr>
		<td class="vtop rowform">
			<yy:TextBox ID="displayorder" runat="server" CanBeNull="必填" RequiredFieldType="数据校验" Text="0" Width="56px" MultiLine="true" MaxLength="7"></yy:TextBox>
		</td>
		<td class="vtop"></td>
	</tr>
	...
	<tr>
		...
		<yy:TextBox ID="code" runat="server" Rows="4" IsReplaceInvertedComma="false" HintTitle="提示"
			HintInfo="请直接输入需要展现的广告的 html 代码" RequiredFieldType="暂无校验" CanBeNull="可为空" TextMode="MultiLine"
			Width="70%" Height="150" HintLeftOffSet="50"></yy:TextBox>
		</td>
	</tr>
	</tbody>
	...
	<tr>
		<td class="vtop rowform">
			<yy:TextBox ID="wordcontent" runat="server" Width="200px" RequiredFieldType="暂无校验" CanBeNull="可为空"></yy:TextBox>
		</td>
	...
	<tr>
		<td class="vtop rowform">
			<yy:TextBox ID="imgtitle" runat="server" Width="344px" RequiredFieldType="暂无校验" CanBeNull="可为空"></yy:TextBox>
		</td>
	...
	<tr>
		<td class="vtop rowform">
			<yy:TextBox ID="imglink" runat="server" Width="250px" RequiredFieldType="暂无校验" CanBeNull="可为空" Text=""></yy:TextBox>
		</td>
...
<div class="Navbutton">
<yy:Button ID="UpdateADInfo" runat="server" Text=" 提 交 " ValidateForm="true"></yy:Button>&nbsp;&nbsp;
<yy:Button ID="DeleteADInfo" runat="server" Text=" 删 除 " ButtonImgUrl="../images/del.gif"></yy:Button>&nbsp;&nbsp;
