...
namespace SAS.Album.Pages
{
    ...
    /// 个人中心管理照片
    /// </summary>
    public class usercpphotomanage : BasePage
    {
        #region 页面变量
        /// <summary>
        /// 当前页码
        ...
        /// 所属相册
        /// </summary>
        public int albumid = SASRequest.GetInt("albumid", 0);
        /// <summary>
        /// 相册列表
        /// </summary>
        ...
        protected override void ShowPage()
        {
            ...
            if (SASRequest.IsPost())
            {
                ...
                string active = SASRequest.GetFormString("active");
                if (active == "delete")
                    DeletePhoto();
            ...
            foreach (DataRow singlePhotoInfo in photoList.Rows)
            {
                ...
                if (singlePhotoInfo["FileName"].ToString().Trim().ToLower().IndexOf("http") == 0)
                    ...
                    singlePhotoInfo["TFileName"] = Globals.GetThumbnailImage(BaseConfigs.GetSitePath + singlePhotoInfo["FileName"].ToString().Trim());
            ...
            pagenumbers = Utils.GetPageNumbers(pageid, pagecount, "usercpmanagephoto.aspx?albumid=" + albumid, 8);
        }

        private void DeletePhoto()
        {
            string photoIdList = SASRequest.GetFormString("photoid");
            ...
            SetUrl("usercpmanagephoto.aspx?albumid=" + albumid);
            SetMetaRefresh();
        ...
        {
            int targetAlbumId = TypeConverter.StrToInt(SASRequest.GetFormString("alibumList"));
        ...
        {
            int photoId = Convert.ToInt32(SASRequest.GetFormString("logoList"));
            if (photoId == -1)
                return;
